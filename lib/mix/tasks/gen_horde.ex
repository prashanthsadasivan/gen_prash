defmodule Mix.Tasks.GenHorde do
  ...
  def run(args, help \\ __MODULE__) do
    module_name = case args do
      ...
      _ -> help.raise_with_help "Expected single argument that is a valid module name. got: #{inspect(args)}"
    end

    camel_case_name = Macro.camelize(module_name)

    [
      {get_node_listener_template(), "node_listener.ex"},
      {get_registry_template(), "horde_registry.ex"},
      {get_supervisor_template(), "horde_supervisor.ex"}
    ]
  ...
  def validate_module_name(module_name) do
    case module_name =~ ~r/^[A-Z]\w*(\.[A-Z]\w*)*$/ do
      true -> module_name
      _ -> help.raise_with_help "Expected single argument that is a valid module name. got: #{inspect(module_name)}"
    end
  end
